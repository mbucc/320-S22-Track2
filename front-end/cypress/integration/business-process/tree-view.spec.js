...
describe('Expand all shows all elements, and collapse all hides all elements.',()=>{
  ...
  it('Expands each node.', ()=>{
    ...
    cy.get('li.eai-domain').each((eai)=>{cy.wrap(eai).contains(TopDomain).click()})
    .children().get('li.publishing-biz-domain').each((pubBiz)=>{cy.wrap(pubBiz).contains(BizDomain).click()})
    .children().get('li.biz-process').each((bizProcess)=>{cy.wrap(bizProcess).contains(BizProcess).click()})
    .children().get('li.tree-log').each((log)=>{cy.wrap(log).click()})
  });
});
